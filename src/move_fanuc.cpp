...
{ //constructor
    ...
    ROS_INFO_NAMED("move_fanuc", "Arm reference frame: %s", planning_frame.c_str());
    ROS_INFO_NAMED("move_fanuc", "Arm end effector link: %s", move_group.getEndEffectorLink().c_str());
    //ROS_INFO_NAMED("move_fanuc", "Link Name: %s", link_names[link_names.size() - 2].c_str());
...
{ //get current pose
    ROS_INFO_NAMED("move_fanuc", "Arm current pose recorded");
    return move_group.getCurrentPose().pose;
}

std::vector<double> Arm::getCurrentJointValues()
{ //get current joint values
    ROS_INFO_NAMED("move_fanuc", "Arm current joint values recorded");
...
{ //get joint value of the joint given by joint_name
    ...
    { //if there is mismatch between number of jaint names and number of jaint values
        ROS_INFO_NAMED("move_fanuc", "Number of joint names different from number of joint_values");
        return -1;
    }
    else
    {
        for (int joint_index = 0; joint_index < current_joint_values.size(); joint_index++)
        {
            ...
            {
                ROS_INFO_NAMED("move_fanuc", "Cuurent %s value %f is obtained", joint_name.c_str(), current_joint_values[joint_index]);
...
{                     //set maximum velocity scaling factor
    if (factor > 1.0) //cannot exceed maximum velocity of the robot
    {
        ...
        ROS_INFO_NAMED("move_fanuc", "Velocity scaling factor greater than 1.0. 1.0 is used");
    }
    else
    {
        move_group.setMaxVelocityScalingFactor(factor);
        ROS_INFO_NAMED("move_fanuc", "Velocity scaling factor set as %.2f", factor);
    }
    vel_factor = factor;
}
...
{ //move to a defined pose, default step = true, default execute = false
    ...
    ROS_INFO_NAMED("move_fanuc", "Visualizing plan to target pose %s", success ? "" : "FAILED");
    ...
    if (execute == true)
    {
        ...
        ROS_INFO_NAMED("move_fanuc", "Plan to target pose exexuted");
        if (step == true)
        {
            visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window");
        }
...
{ //move to a target pose with constraint
    ...
    { //use orientation constraint as the target orientation, otherwise there will be error
        ROS_INFO_NAMED("move_fanuc", "Orientaiton constraint will be set as target orientation");
...
{ //move to target joint values, default step = true, default execute = false
    ...
    success = (move_group.plan(plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
    ROS_INFO_NAMED("move_fanuc", "Visualizing plan to target joint values %s", success ? "" : "FAILED");
    visual_tools.trigger();
    ...
    {
        move_group.execute(plan);
        ROS_INFO_NAMED("move_fanuc", "Plan to target joint values exexuted");
        if (step == true)
        {
            visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window");
        }
...
{                                                      //set orienation constraint to current end effector orientation
    ocm.link_name = link_names[link_names.size() - 1]; //assign link_6 as the ocm link name
    ...
    ROS_INFO_NAMED("move_fanuc", "End effector orientation constraint set");
...
{ //set joint value constraint to the current joint value of a given joint
    ...
    ROS_INFO_NAMED("move_fanuc", "%s value constraint set", joint_name.c_str());
...
void Arm::clearPathConstraint()
{
    ...
    ROS_INFO_NAMED("move_fanuc", "Path constraint clear");
...
{ //set waypoints using intermediate poses
    ...
    { //if set_waypoints == false, that means waypoints haven't been set.
        ...
        set_waypoints = true;
        ROS_INFO_NAMED("move_fanuc", "Current pose pushed to waypoints");
    }
    waypoints.push_back(inter_pose);
    ROS_INFO_NAMED("move_fanuc", "Intermediate pose pushed to waypoints");
}
...
{ //move robot according to cartesian path
    if (waypoints.size() == 0)
    {
        ROS_INFO_NAMED("move_fanuc", "No waypoints found");
        return;
    }
    ...
    ROS_INFO_NAMED("move_fanuc", "Visualizing cartesian path (%.2f%% acheived)", fraction * 100.0);
    ...
    if (scale_traj == true)
    {
        ROS_INFO_NAMED("move_fanuc", "Trajectory scaled according to velocity factor %.2f", vel_factor);
    }
    else
    {
        ROS_INFO_NAMED("move_fanuc", "Trajecotry scaling failed. Velocity factor = 1.0 is used");
    }
    ...
    {
        move_group.execute(plan);
        ROS_INFO_NAMED("move_fanuc", "Cartesian path exexuted");
        if (step == true)
        {
            visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window");
        }
    ...
    waypoints.clear();
    set_waypoints = false;
    ROS_INFO_NAMED("move_fanuc", "Waypoints cleared");
}
} // namespace fanuc
